      ...
      <div className="relative w-full max-w-[1200px] overflow-hidden rounded-3xl bg-white/80 backdrop-blur-xl shadow-2xl shadow-orange-100/50 border border-white/60">
        ...
        <div className="bg-gradient-to-r from-orange-500 via-orange-400 to-amber-400 px-8 py-7 relative overflow-hidden">
          ...
          <div className="relative z-10 flex gap-2 mt-6">
            {([['OVERVIEW', '📊', 'Genel Bakış'], ['USERS', '👥', 'Kullanıcılar'], ['REPORTS', '🚨', 'Şikayetler']] as [Tab, string, string][]).map(([key, icon, label]) => (
              <button key={key} onClick={() => setActiveTab(key)}
                className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold transition-all ${activeTab === key ? 'bg-white text-orange-600 shadow-lg' : 'text-white/80 hover:bg-white/20'}`}>
                ...
                <div className="animate-fadeIn space-y-6">
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    {[
                      ...
                      { label: 'Dondurulmuş', value: frozenUsers, icon: '🧊', color: 'from-slate-500 to-slate-600', bg: 'bg-slate-50' },
                        ...
                        <tbody className="divide-y divide-gray-50">
                          {users.map((u) => (
                            <tr key={u.id} className="hover:bg-orange-50/30 transition-colors">
                              ...
                              <td className="px-6 py-4">
                                <span className={`px-2.5 py-1 rounded-lg text-[10px] font-bold ${u.role === 'ADMIN' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700'}`}>
